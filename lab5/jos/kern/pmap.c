...
boot_alloc(uint32_t n)
{
	...
	cprintf("need boot_alloc:0x%x bytes\t nextfree:0x%x\n", n, nextfree);
...
mem_init(void)
{
  ...
  memset(envs, 0, NENV * sizeof(struct Env));
	...
	// 检查page_free_list是否正确
	check_page_free_list(1);
	
	// 检查page_alloc、page_free这两个函数是否会运行成功
	check_page_alloc();
	// 检查两级的页表转换是否会运行成功
	// 主要检查pgdir_walk、page_lookup、page_remove、page_insert函数是否OK
	check_page();
	...
	// Your code goes here:
	//cprintf("boot_map_region 0x%x   0x%x\n)", pages, PADDR(pages));
  ...
  boot_map_region(kern_pgdir, UENVS, PTSIZE, PADDR(envs), PTE_U);  
	...
	// Your code goes here:
	//cprintf("boot_map_region KSTACK 0x%x\n", PADDR(bootstack));
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  for(; start < end; start += PGSIZE ){
    ...
    if((uint32_t)ULIM < (uint32_t)start || pte == NULL || ((*pte & perm) != perm) ){
